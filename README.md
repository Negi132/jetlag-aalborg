...
POI-based Measuring keeps a movable seeker position. Rail-station Measuring automatically selects the scheduled passenger station nearest that position. Coastline Measuring treats both Limfjord banks as one category: the asker's nearest shoreline on either bank establishes the reference distance, then the same threshold is applied around both banks. The shoreline is intentionally **generalized rather than exact**: the weekly build uses only formal OSM `natural=coastline`, samples both banks at medium detail, smooths only small harbour/vertex excursions, and keeps the general bends of the fjord. Roads, railways, promenades, quays and embankments are never eligible coastline sources. Body-of-water Measuring still shows every cached target and preserves distinct unnamed lakes/ponds. The weekly build also stores each target marker/bounds and emits a more aggressively simplified pre-unioned distance geometry, so opening the card does no per-water clipping/point-on-feature work and repositioning buffers only one or two compact geometries. Named chalk/limestone quarries remain a fallback when OSM has no separate water polygon. Zone-border, coastline, water, POI, and generic Measuring previews remain drafts until **Log answer** and can be repositioned before logging.
...
The weekly hydro snapshot now deliberately uses **only** `natural=coastline` for coastline. It converts the detailed OSM shore into two medium-detail Limfjord-bank polylines. The smoothing window is deliberately short enough to follow the general boundary of both shores, while still bridging small harbour/marina indentations and ignoring urban infrastructure tags entirely. Unnamed OSM water features keep stable per-object identities, named chalk/limestone quarry polygons are only used when no better mapped water geometry exists inside them, and a separate pre-unioned water-distance cache reduces runtime buffer work.


### v4.9 hydro performance
