...
### The best option: Aalborg's own live boundaries

Everything below about tracing and placement is a **stand-in**. The real source is Aalborg's KortInfo
WFS, and if it will talk to your browser you should use it instead — live geometry arrives with real
coordinates, so there is no tracing error and no placement step at all.

> **Layers → Place the zones → Try Aalborg's live boundaries instead**

That points zones 2 and 3 at `drift.kortinfo.net/Wfs.aspx?Site=Aalborg&Page=kortHjemmeside`, asks it
which layers it will share, and opens the picker. Look for something like *bydele* or
*kommuneplanomraade*. KortInfo answers in GML rather than GeoJSON, which the app now parses, and it
publishes in UTM zone 32, which the app already reprojects.

I could not test this end to end: from here that host returns an OGC exception document I can't read,
and another of its pages returns 401. KortInfo's WFS generally needs the site administrator to have
enabled sharing, and a URL generated by KortInfo's own *Linkgenerator*. If you obtain one, paste it
behind the ⚙ on a zone level and it will be used in place of the traced outlines. The GML parsing,
the reprojection and the fallback are all tested against fixtures; only whether the server answers is
unknown.
...
node ui.test.mjs         # 204 checks — the app, driven headlessly
```
