...
POI-based Measuring keeps a movable seeker position. Rail-station Measuring automatically selects the scheduled passenger station nearest that position. Coastline Measuring treats both Limfjord banks as one category: the asker's nearest shoreline on either bank establishes the reference distance, then the same threshold is applied around both banks. The shoreline is intentionally **abstract**: the weekly build uses only formal OSM `natural=coastline`, samples the fjord-facing envelope of each bank, smooths harbour indentations, and simplifies each bank to a small polyline. Roads, railways, promenades, quays and embankments are never eligible coastline sources. Body-of-water Measuring still shows every cached target and preserves distinct unnamed lakes/ponds, but the weekly build also emits a pre-unioned/simplified distance geometry so repositioning buffers only one or two compact geometries instead of every water body separately. Named chalk/limestone quarries remain a fallback when OSM has no separate water polygon. Zone-border, coastline, water, POI, and generic Measuring previews remain drafts until **Log answer** and can be repositioned before logging.
...
The weekly hydro snapshot now deliberately uses **only** `natural=coastline` for coastline. It converts the detailed OSM shore into two broad, simplified Limfjord-bank polylines and deliberately bridges small harbour/marina indentations. This sacrifices unnecessary harbour-detail precision in exchange for a much smaller, faster and far harder-to-contaminate coastline model. Unnamed OSM water features keep stable per-object identities, named chalk/limestone quarry polygons are only used when no better mapped water geometry exists inside them, and a separate pre-unioned water-distance cache reduces runtime buffer work.
